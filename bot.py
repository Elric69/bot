...
#To change proxy if return forbidden 

from flask import Flask, jsonify
app = Flask("")
response = requests.get("https://www.docserver.rf.gd/gen-proxy")
p=response.text
...
# Counter to keep track of the current proxy
proxy_index = 0
...
def use_proxy():
    try:
        ...
        if response.status_code == 403:
            switch_proxy()
            return jsonify({"message": "Proxy switch"}), 403
        return jsonify(response.json())
    except:
        # If the request fails, switch to the next proxy
        switch_proxy()
        return jsonify({"message": "Proxy switch"}), 500
...
game = "http://aminoapps.com/p/5wzaa6"
...
p = client.get_from_code(game).objectId
gameid.append(p)
...
def play(chatId : None):
	data = {
    comId,
    chatId,
    "/path/to/video",
    "Title",
    open("someFile.jpg", "rb"),
...
def on_chat_tip(data):
	...
	if authorId in gambler:
		if i == "10":
			startGam.append(authorId)
			sub.send_message(chatId=data.message.chatId,message="ace\nking\nqueen\n\nNow choose one of the above and use this command -start\nEx : -start ace")
		elif i != "10":
			blog = sub.get_user_blogs(authorId).blogId
			b1 = blog[0]
			sub.send_coins(coins= i ,blogId = b1)
			sub.send_message(chatId=data.message.chatId,message="Please send only 10 coins to start the game")
	elif authorId == client.userId:
			pass
	else:
...
def on_text_message(data):
	...
	if author.userId in blackList:pass
		...
		if command("-balance"):
			cos=int(client.get_wallet_info().totalCoins)
			sub.send_message(chatId,f"Total coins {cos}",replyTo=msgId)

		if command("-gamble"):
			if chatId in gameid:
				cos=int(client.get_wallet_info().totalCoins)
				if cos < 10:
					sub.send_message(chatId,"Bot don't have sufficient balance for gambling",replyTo=msgId)
				if cos > 10:
					blog = sub.get_user_blogs(authorId).blogId
					log = len(blog)
					if log == 0:
						sub.send_message(chatId=chatId,message="Please create a blog first",replyTo=data.message.messageId)
					else:
						gambler.append(authorId)
						sub.send_message(chatId = chatId, message= "Now send coins to the host \nNote -> \n1.send only 10 coins\n2.Before you send coins make sure you already created a blog")
			if chatId not in gameid:
				sub.send_message(chatId = chatId,message=f"{game}\n\n join this chat to use this command")

		if command("-start"):
			if chatId in gameid:
				if authorId not in startGam:
					sub.send_message(chatId = chatId,message="You haven't gambled coins yet use -gamble to bet the coins")
				if authorId in startGam:
					ch = ["ace","king","queen"]
					...
					m = random.choice(ch)
					if m == player :
						startGam.remove(authorId)
						gambler.remove(authorId)
						sub.send_message(chatId,"Congrats You win!",replyTo=data.message.messageId)
						blog = sub.get_user_blogs(authorId).blogId
						b2 = blog[0]
						sub.send_coins(coins=20,blogId = b2)
						sub.send_message(chatId,message=f"20 coins sent to {author.nickname}")
					else:
						startGam.remove(authorId)
						gambler.remove(authorId)
						sub.send_message(chatId,"You lose!\nTry again",replyTo=data.message.messageId)
			if chatId not in gameid:
				sub.send_message(chatId = chatId,message=f"{game}\n\n join this chat to use this command")
